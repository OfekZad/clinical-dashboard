...
type CachedDashboardScore = {
  ...
  data: Pick<
    Assessment,
    | "id"
    | "has_screen_intolerance"
    | "has_night_driving_issues"
    | "has_wind_sensitivity"
    | "has_low_humidity_issues"
  >;
};

type ScoredCachedSurvey = PatientSurvey & {
  patient_id: string;
  total_score: number;
  severity_level: Assessment["severity_level"];
};

function isScoredPatientSurvey(
  survey: PatientSurvey,
): survey is ScoredCachedSurvey {
  return Boolean(
    survey.patient_id &&
    survey.status === "scored" &&
    survey.total_score !== null &&
    survey.severity_level !== null,
  );
}
...
export function useCachedAssessmentResponses(
  assessmentId: string,
): AssessmentResponse[] {
  const { cache } = useDataCache();
  return useMemo(
    () =>
      Object.values(cache.assessment_responses).filter(
...
export function useCachedClinicianNotes(assessmentId: string): ClinicianNote[] {
  ...
  return useMemo(
    () =>
      Object.values(cache.clinician_notes).filter(
...
export function useCachedSurveyResponses(surveyId: string): SurveyResponse[] {
  ...
  return useMemo(
    () =>
      Object.values(cache.survey_responses).filter(
...
export function useCachedMedications(patientId: string): PatientMedication[] {
  ...
  return useMemo(
    () =>
      Object.values(cache.patient_medications).filter(
...
export function useCachedDashboardPatients(): PatientWithLatestAssessment[] {
  ...
  return useMemo(() => {
    ...
    const surveys = Object.values(cache.patient_surveys).filter(
      isScoredPatientSurvey,
    );

    // Group by patient_id
    const byPatient: Record<string, CachedDashboardScore[]> = {};
      ...
      return {
        ...
        latest_assessment: latest
          ? {
              ...
              has_screen_intolerance:
                latest.data.has_screen_intolerance ?? false,
              has_night_driving_issues:
                latest.data.has_night_driving_issues ?? false,
              has_wind_sensitivity: latest.data.has_wind_sensitivity ?? false,
              has_low_humidity_issues:
                latest.data.has_low_humidity_issues ?? false,
            }
          : null,
        previous_assessment: previous ? { total_score: previous.score } : null,
      };
    });
